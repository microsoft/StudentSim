...
published per-player files come already in this draw, so this is for anyone
...
roster, so a given seed selects the same 1,000 records without shipping them.
Two details must not be tidied away: the modes are drawn in sorted order, and
the whole selection is shuffled afterwards. Both decide the order the generator
is consumed in, and changing either gives a different thousand records for the
same seed.
"""
