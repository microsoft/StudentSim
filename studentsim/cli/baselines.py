...
def prompt_and_answer(record: dict, *, turn: int) -> tuple[str, str]:
...
def build_simulator(name: str, *, domain: str, model: str | None, elo: int, device: str):
    ...
    if name == "azure":
        ...
        return LLMClientSimulator(
            client=AzureOpenAIClient(deployment=model), domain=domain, top_logprobs_k=top_k
        )
    if name == "qwen3":
...
def score_fidelity(simulator, records: Sequence[dict], *, domain: str, decoding) -> dict:
    """The share of first turns the baseline reproduces."""
    if domain == "math":
        rows, answers = [], []
        for record in records:
            prompt, answer = prompt_and_answer(record, turn=0)
    ...
    pairs = [prompt_and_answer(record, turn=0) for record in records]
...
def score_responsiveness(simulator, records: Sequence[dict], *, domain: str, decoding) -> dict:
    ...
    pairs, modes = [], []
    for record in records:
        prompt, answer = prompt_and_answer(record, turn=1)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.single_turn is not None:
        ...
        report["fidelity"] = score_fidelity(
            simulator, read_records(args.single_turn), domain=args.domain, decoding=decoding
        )
    if args.multi_turn is not None:
        ...
        report["responsiveness"] = score_responsiveness(
            simulator, read_records(args.multi_turn), domain=args.domain, decoding=decoding
        )
